...
CryptoPP::SecByteBlock crypto_operator::decrypt(const CryptoPP::SecByteBlock& _symmetric_key, const cfb_encrypted_data& _cfb_encrypted_data) const {
    // Calculate a SHA-256 hash over the Diffie-Hellman session key
    CryptoPP::SecByteBlock key(CryptoPP::SHA256::DIGESTSIZE);
    CryptoPP::SHA256().CalculateDigest(key, _symmetric_key, _symmetric_key.SizeInBytes());
    // Decrypt
...
std::vector<CryptoPP::byte> crypto_operator::sign(const CryptoPP::PrivateKey& _private_key, std::vector<CryptoPP::byte> _data) {
    ...
    CryptoPP::RSASSA_PKCS1v15_SHA256_Signer signer(_private_key);
...
bool crypto_operator::verify(CryptoPP::PublicKey& _public_key, std::vector<CryptoPP::byte> _data) {
    ...
    CryptoPP::RSASSA_PKCS1v15_SHA256_Verifier verifier(_public_key);
    try {
