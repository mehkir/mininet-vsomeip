...
service_discovery_impl::create_eventgroup_entry(
    ...
    process_authentication_for_created_subscribe(its_data, _reliability_type, its_reliable_endpoint, its_unreliable_endpoint, _service, _instance);
...
#ifdef WITH_SERVICE_AUTHENTICATION
void
service_discovery_impl::process_authentication_for_created_subscribe(
    ...
    if (its_data.entry_->get_type() == entry_type_e::SUBSCRIBE_EVENTGROUP && its_data.entry_->get_ttl() > 0) {
        ...
        VSOMEIP_DEBUG << __func__ << " CLIENT SIGN START";
        ...
        if (signed_nonce_vector.empty()) {
            // TODO: Better would be to return here and check on the receiving node if the required conf option is included
...
service_discovery_impl::process_authentication_for_created_subscribe_ack(ttl_t _ttl, boost::asio::ip::address_v4 _subscriber_address, service_t _service, instance_t _instance, major_version_t _major, vsomeip_v3::sd::entry_data_t& _its_data) {
    ...
    if(_ttl) {
        ...
        std::vector<CryptoPP::byte> signature = crypto_operator_.sign(private_key_, data_to_be_signed);
        data_partitioner().partition_data<std::vector<unsigned char>>(SIGNATURE_CONFIG_OPTION_KEY, configuration_option, signature);
        VSOMEIP_DEBUG << __func__ << " SERVICE SIGN END";
...
#if defined(WITH_DNSSEC) && defined(NO_SOMEIP_SD)
...
service_discovery_impl::mimic_offerservice_serviceentry(
    ...
    ttl_t ttl = VSOMEIP_SD_DEFAULT_TTL;
...
service_discovery_impl::process_offerservice_serviceentry(
    ...
    // << std::endl;
    // Addition for checking parameter contents End ################################################################
    std::shared_ptr < runtime > its_runtime = runtime_.lock();
    if (!its_runtime)
        return;
...
service_discovery_impl::validate_offer(service_t _service, instance_t _instance, major_version_t _major, minor_version_t _minor) {
    ...
    VSOMEIP_DEBUG << __func__ << " VALIDATE OFFER END";
#endif
    ...
    if (entry_type_e::SUBSCRIBE_EVENTGROUP == its_type) {
        ...
        if (entry_type_e::SUBSCRIBE_EVENTGROUP_ACK == its_type) { //this type is used for ACK and NACK messages
            if (its_ttl > 0) {
                ...
                validate_subscribe_ack_and_verify_signature(_sender.to_v4(), its_service, its_instance, its_major);
#else
...
service_discovery_impl::on_find_debounce_timer_expired(
    ...
    VSOMEIP_DEBUG << __func__ << " FIND SEND";
