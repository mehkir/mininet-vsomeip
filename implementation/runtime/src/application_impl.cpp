...
std::shared_ptr<payload> application_impl::decrypt_payload(service_t _service, instance_t _instance, cfb_encrypted_data _cfb_encrypteddata) const {
    auto key_tuple = std::tuple<service_t, instance_t>(_service, _instance);
    CryptoPP::SecByteBlock symmetric_key(group_secrets_.operator*()[key_tuple]);
    CryptoPP::SecByteBlock decrypted_payload_data = crypto_operator_.decrypt(symmetric_key, _cfb_encrypteddata);
    std::vector<unsigned char> decrypted_payload_vector(decrypted_payload_data.begin(), decrypted_payload_data.end());
    std::shared_ptr<payload> decrypted_payload = runtime::get()->create_payload();
    decrypted_payload->set_data(decrypted_payload_vector);
    return decrypted_payload;
}

std::shared_ptr<payload> application_impl::encode_payload(cfb_encrypted_data _cfb_encrypteddata) const {
    payload_key_value_encoder_.add_item(INITIALIZATION_VECTOR_PAYLOAD_KEY_NAME, _cfb_encrypteddata.initialization_vector_);
    payload_key_value_encoder_.add_item(ENCRYPTED_DATA_PAYLOAD_KEY_NAME, std::vector<unsigned char>(_cfb_encrypteddata.encrypted_data_.begin(), _cfb_encrypteddata.encrypted_data_.end()));
...
cfb_encrypted_data application_impl::decode_payload(std::shared_ptr<payload> _payload) const {
