...
class my_service_sample {
public:
    my_service_sample(bool _use_tcp, uint32_t _cycle) :
            ...
            blocked_(true),
            running_(true),
            notify_thread_(std::bind(&my_service_sample::notify, this)) {
    }
    ...
    bool init() {
        ...
        app_->register_state_handler(
                std::bind(&my_service_sample::on_state, this,
                        std::placeholders::_1));
        ...
        app_->register_message_handler(
                ...
                std::bind(&my_service_sample::on_get, this,
                          std::placeholders::_1));
        ...
        app_->register_message_handler(
                ...
                std::bind(&my_service_sample::on_set, this,
                          std::placeholders::_1));
    ...
    void notify() {
        ...
        {
            std::unique_lock<std::mutex> blocked_lock(mutex_);
            condition_.wait(blocked_lock, [this]{return !blocked_;});
        }
...
#ifndef VSOMEIP_ENABLE_SIGNAL_HANDLING
    my_service_sample *its_sample_ptr(nullptr);
...
int main(int argc, char **argv) {
    ...
    my_service_sample its_sample(use_tcp, cycle);
#ifndef VSOMEIP_ENABLE_SIGNAL_HANDLING
